...
void App::run()
{
    ...
    while (not glfwWindowShouldClose(this->wnd))
    {
		...
		et = std::chrono::duration<float>(std::chrono::steady_clock::now() - beg).count();
    }
}
const glm::mat4& App::get_camera_matrix()
{
    return this->camera.get_matrix();
}
...
void App::draw_cube(Shader& shader, glm::vec3 pos, glm::vec4 color, GLfloat size, GLfloat rot, glm::vec3 norm)
{
	...
	static const bool dummy = (va.link_attrib(vb, 0, 4, GL_FLOAT, sizeof(glm::vec4), (const void*)0), false);
	...
	glm::mat4 mvp = get_camera_matrix() * glm::translate(glm::rotate(glm::scale(model, glm::vec3(size, size, size)), rot, norm), pos);
