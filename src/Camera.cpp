...
void Camera::handle_inputs(GLFWwindow* window)
{
	...
	if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
	{
		...
		float roty = sensitivity * (float)(mouseX - (float(height) / 2.f)) / float(height);

		glm::vec3 newOrientation = glm::rotate(Orientation, glm::radians(-rotx), glm::normalize(glm::cross(Orientation, Up)));

		if (not ((glm::angle(newOrientation, Up) <= glm::radians(5.f)) or (glm::angle(newOrientation, -Up) <= glm::radians(5.f))))
		{
