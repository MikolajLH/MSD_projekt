                ...
                for (int s = 0; s < N; ++s)
                    for (int r = 0; r < N - 1; ++r)
                        for (int c = 0; c < N; ++c)
                        {
                            ...
                            NU.temperature += prim * (cell.temperature - upper.temperature) * dt;
                            foreground.set_if(s, r + 1, c, NU);

                            //foreground(s, r + 1, c).temperature += prim * (cell.temperature - upper.temperature) * dt;
                            foreground(s, r, c).temperature -= prim * (cell.temperature - upper.temperature) * dt;
                        }
                std::cout << foreground(0, 0, 0).temperature << "      " << foreground(0, N - 1, 0).temperature << "\n";
