...
class Demo : public App
{
	public:
		Demo(int w, int h, std::string_view title)
			: App(w, h, title, glm::vec3(10, 0, 20)), cube_shader("res/shaders/cube.shader")
		{
			...
			for (size_t s = 0u; s < lattice.N; ++s)
				for (size_t r = 0u; r < lattice.N; ++r)
					for (size_t c = 0u; c < lattice.N; ++c)
					{
						...
						if (lattice(s, r, c).type == air)continue;
						if (glfwGetKey(wnd, GLFW_KEY_Q) == GLFW_PRESS)
						{
							...
							if (celciusT < 0.f)
							{
								color = glm::vec4(1.f + celciusT / 20.f, 1.f + celciusT / 20.f, 1.f, 1.f);
							}
							else
							{
								color = glm::vec4(1.f, 1.f - celciusT / 100.f, 1.f - celciusT / 100.f, 1.f);
							}

						}
						else
						{
							if (lattice(s, r, c).type == concrete)color = clr::gray;
							else if (lattice(s, r, c).type == wood)color = clr::brown;
